...
export default function RegisterModal({ onClose, onSwitchToLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true);
    try {
      await register({ username, email, password, clubId: clubId || undefined });
      onClose();
    } catch (err) {
      setError(err.response?.data?.message || err.response?.data?.error || 'No pudimos crear tu cuenta. Probá de nuevo.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-[1300] flex items-end justify-center gc-overlay" style={{ backgroundColor: rgba('#000000', 0.6) }} onClick={onClose}>
      <div
        ...
        style={{ backgroundColor: C.bg, border: `1px solid ${C.border}`, borderBottom: 'none', maxHeight: '90vh' }}
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="px-5 pb-6 pt-4 space-y-4">
          ...
          <p className="text-center text-sm" style={{ color: C.muted }}>
            ¿Ya tenés cuenta?{' '}
            <button type="button" onClick={onSwitchToLogin} className="font-semibold gc-focus" style={{ color: C.brandBright }}>
              Iniciá sesión
            </button>
          </p>
        </form>
